...
use Fezz\Acube\Sdk\Italy\Invoice\Requests\GetDraftInvoiceRequest;

test('resolveEndpoint returns correct path', function (): void {
    $dto = new GetDraftInvoiceRequestDto(id: 'draft-123');
    $request = new GetDraftInvoiceRequest($dto);

    expect($request->resolveEndpoint())->toBe('/invoices/draft/draft-123');
});
