...
use Fezz\Acube\Sdk\Italy\Invoice\Requests\NotifyInvoiceRequest;

test('resolveEndpoint returns correct path', function (): void {
    $dto = new NotifyInvoiceRequestDto(id: 'invoice-123');
    $request = new NotifyInvoiceRequest($dto);

    expect($request->resolveEndpoint())->toBe('/invoices/invoice-123/notify');
});
