...
test('resolveEndpoint returns correct path', function (): void {
    $request = new VerifySplitRequest('company-123');

    expect($request->resolveEndpoint())->toBe('/verify/split/company-123');
});
...
test('createDtoFromResponse returns array', function (): void {
    ...
    $mockClient = new MockClient([
        MockResponse::make(['split_payment' => true], 200),
    ]);
    ...
    expect($result)->toBeArray()
        ->and($result)->toHaveKey('split_payment');
});
